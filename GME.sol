...
contract GME is Context, IERC20, Ownable {
    ...
    uint256 private constant TOTAL_SUPPLY = 10000000000 * 10**1 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % TOTAL_SUPPLY));
    uint256 private _tFeeTotal;

    string private constant NAME = "Game Coin";
    string private constant SYMBOL = "GME";
    ...
    uint256 private NUMBER_OF_TOKENS_TO_SELL_TO_ADD_TO_LIQUIDITY = 30000000 * 10**1 * 10**9;
    ...
    event FeeExclusionUpdated(address account, bool enabled);
    event LiquidityFeePercentUpdated(uint256 oldLiquidityFee, uint256 liquidityFee);
    event MaxTaxAmountUpdated(uint256 oldMaxTaxAmount, uint256 maxTaxAmount);
    event CharityWalletAddressUpdated(address oldCharityWalletAddress, address charityWalletAddress);
    event MarketingWalletAddressUpdated(address oldMarketingWalletAddress, address marketingWalletAddress);
    ...
    function excludeFromFee(address account) public onlyOwner {
        emit FeeExclusionUpdated(account, true);
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        emit LiquidityFeePercentUpdated(_liquidityFee, liquidityFee);
        _liquidityFee = liquidityFee;
    }

    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        uint256 maxTxAmount = TOTAL_SUPPLY.mul(maxTxPercent).div(
            10**2
        );

        emit MaxTaxAmountUpdated(_maxTxAmount, maxTxAmount);
    ...
    function _setCharityWallet(address payable charityWalletAddress) external onlyOwner() {
        ...
        emit CharityWalletAddressUpdated(_charityWalletAddress, charityWalletAddress);
        _charityWalletAddress = charityWalletAddress;
    }
    ...
    function _setMarketingWallet(address payable marketingWalletAddress) external onlyOwner() {
      ...
      emit MarketingWalletAddressUpdated(_marketingWalletAddress, marketingWalletAddress);
      _marketingWalletAddress = marketingWalletAddress;
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= NUMBER_OF_TOKENS_TO_SELL_TO_ADD_TO_LIQUIDITY;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            ...
            contractTokenBalance = NUMBER_OF_TOKENS_TO_SELL_TO_ADD_TO_LIQUIDITY;
            //add liquidity
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        sendBNBToCharity(portionForFees.mul(2).div(3));
